...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  ...
  [self.server add:@"/"
             block:^(CRRequest *request, CRResponse *response,
                     CRRouteCompletionBlock completionHandler) {
               [response finish];
             }];

  // GET '/user/:id' return status code 200 with the id
  [self.server add:@"/user/:id"
             block:^(CRRequest *request, CRResponse *response,
                     CRRouteCompletionBlock completionHandler) {
  ...
  NSError *error;
  if (![self.server startListening:&error portNumber:8080]) {
...
- (CRApplicationTerminateReply)applicationShouldTerminate:
    (CRApplication *)sender {
