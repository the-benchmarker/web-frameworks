...
def commands_for(language, framework, variant, provider = 'docker')
  ...
  concurrencies.split(',').each do |concurrency|
    ...
    routes.split(',').each do |route|
      ...
      zrk_cmds << "#{zrk_path} --plain -c #{concurrency} -d #{duration} -m #{method} --format json --output #{output} -R1000:500000 --interval 1s --timeout 15s --latency --format json http://`cat #{hostname}`:3000#{uri}"
    end
