...
(define (get-user-handler request body)
  ...
  (define user-id (string-drop path (1+ (string-length user-prefix))))
  (values
   (build-response
    #:headers (add-security-headers `((content-type . (text/plain))))
    #:code 200)
...
;; configure-server: Configure production-grade Guile-Web server
;; @returns server configuration list
...
(define (main args)
  ...
  (let ((server-config (configure-server)))
    
    ;; Start server with error handling
    (catch 'system-error
      (lambda ()
        (format (current-error-port) "Starting Guile-Web benchmark server on port ~a~%" 
                (cadr (assoc 'port (cddr (cadr server-config)))))
        (run-server dispatch-request 'http server-config))
