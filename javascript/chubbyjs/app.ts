        ...
        new PathToRegexpRouteMatcher(
            new Routes([
                Route.get(
                    '/',
                    'index',
                    new CallbackRequestHandler((): ResponseInterface => {
                ...
                Route.get(
                    '/user/:id',
                    'user_view',
                    new CallbackRequestHandler((request: ServerRequestInterface): ResponseInterface => {
                        const response = responseFactory.createResponse(200);
...
const psrRequestFactory = new PsrRequestFactory(new ServerRequestFactory(), new UriFactory(), new StreamFactory());
