...
if [ "$#" -eq 0 ]; then
	find . -mindepth 3 -type f -name config.yaml | grep -Ev 'fibers|lapis|caprese|imi-swoole|laravel-s-laravel|sunrise-router-roadrunner|sw-fw-less|fastpysgi-asgi' > ~/list.txt
else
	COUNT=`echo $1 | grep -c "/"`
	if [ "$COUNT" -eq 0 ]; then
